...
@section('page-name', 'jam')

@section('content')

<h4 class="mt-1 mb-1">Daftar Jam</h4>
<hr class="thick-hr" style="margin-bottom: 75px;">

<div class="d-flex justify-content-between align-items-center mb-3">
    <a href="jam-add" class="btn btn-primary d-flex align-items-center">
...
<div class="table-responsive mt-1">
    ...
    <tbody>
        @foreach ($jamList as $data)
        <tr>
            <td>{{$loop->iteration }}</td>
            <td>{{$data->range_jam}}</td>
            <td class="text-center">
                <div class="d-inline-flex">
                    <a href="jam-edit/{{ $data->id }}" class="btn btn-warning btn-sm d-flex align-items-center me-2">
            ...
            <div class="modal-dialog">
                <div class="modal-content text-center"> <!-- Tambahkan text-center di sini -->
                    ...
                    <div class="modal-body">
                        <p>Apakah Anda yakin ingin menghapus data jam</p> <p><strong>{{ $data->range_jam }}</strong></p>?
                    </div>
                    <div class="modal-footer d-flex justify-content-center"> <!-- Tambahkan d-flex justify-content-center di sini -->
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <form action="{{ url('/jam-destroy/' . $data->id) }}" method="post" class="d-inline">
                            @csrf
                            @method('delete')
