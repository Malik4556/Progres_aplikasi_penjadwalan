...
<h4 class="mt-4 mb-3">Daftar Jumlah Mahasiswa</h4>
...
<div class="card mb-4">
    ...
    <div class="card-body d-flex justify-content-between align-items-center">
        ...
        <button type="button" class="btn btn-primary" onclick="updateStatus()">
            <span id="status-button-text">{{ $status ? 'Tandai Belum Selesai' : 'Tandai Selesai' }}</span>
        </button>
    </div>
</div>

<div class="d-flex justify-content-between align-items-center mb-4">
    <a href="{{ url('mahasiswa-add') }}" class="btn btn-primary d-flex align-items-center">
        <i class="lni lni-circle-plus me-2"></i> Tambah Data
    </a>
    <form action="{{ url('mahasiswa-search') }}" method="get" class="d-flex">
    ...
    <table class="table table-striped table-bordered custom-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $data->angkatan }}</td>
                <td>{{ $data->semester }}</td>
                <td class="text-center">{{ $data->jml_mahasiswa }}</td>
                <td class="text-center">
                    <div class="d-inline-flex">
                        <a href="{{ url('mahasiswa-edit/' . $data->id) }}" class="btn btn-warning btn-sm me-2">
                            <i class="lni lni-pencil me-2"></i> Edit
                        </a>
                        <button class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal{{ $data->id }}">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="confirmDeleteModalLabel{{ $data->id }}">Konfirmasi Hapus</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body text-center">
                            <p>Apakah Anda yakin ingin menghapus data jumlah mahasiswa angkatan <strong>{{ $data->angkatan }}</strong>?</p>
                        </div>
                        <div class="modal-footer justify-content-center">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                            <form action="{{ url('mahasiswa-destroy/' . $data->id) }}" method="post" class="d-inline">
                                @csrf
                                @method('delete')
