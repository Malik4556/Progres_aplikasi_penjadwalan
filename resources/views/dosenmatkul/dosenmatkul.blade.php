...
<h4 class="mt-1 mb-1">Daftar Kesediaan Dosen-Mata Kuliah</h4>
...
<div class="card mb-4">
    ...
    <div class="card-body d-flex justify-content-between align-items-center">
        ...
        <button type="button" class="btn btn-primary" onclick="updateStatus()">
            <span id="status-button-text">{{ $statusdoskul ? 'Tandai Belum Selesai' : 'Tandai Selesai' }}</span>
        </button>
    </div>
</div>

<div class="d-flex justify-content-between align-items-center mb-3">
    <a href="dosenmatkul-add" class="btn btn-primary d-flex align-items-center">
            ...
            <tr>
                ...
                <td class="text-center">
                    <div class="d-inline-flex">
                        <a href="dosenmatkul-edit/{{ $data->id }}" class="btn btn-warning btn-sm d-flex align-items-center me-2">
                ...
                <div class="modal-dialog">
                    <div class="modal-content text-center"> <!-- Tambahkan text-center di sini -->
                        ...
                        <div class="modal-body">
                            <p>Apakah Anda yakin ingin menghapus data kesediaan dosen</p> <p><strong>{{ $data->dosen->nama }}</strong></p>
                            <p> mengampu matakuliah dosen</p> <p><strong>{{ $data->matakuliah->nama }}</strong></p>?
                        </div>
                        <div class="modal-footer d-flex justify-content-center"> <!-- Tambahkan d-flex justify-content-center di sini -->
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                            <form action="{{ url('/dosenmatkul-destroy/' . $data->id) }}" method="post" class="d-inline">
                                @csrf
                                @method('delete')
...
<script>
    function updateStatus() {
        ...
        xhr.open('POST', '{{ url('/update-status-dosenmatkul') }}', true);
